...
- Work 詳細の本文は、ビルド時に意味のまとまりごとの `<span>` に自動分割されます。画面幅が変わっても、語の途中で不自然に改行しにくい状態になります。
- `content.json` の本文を更新したら、必ず 375px / 768px / デスクトップ幅で表示を確認します。改行位置に違和感があれば、対象の text block に `bodySpans` を追加して調整します。1 span は意味のまとまりで区切り、375px 幅で収まる 18 文字前後を目安にします。固有名詞、英単語、リンクは分割しません。
- `bodySpans` は `body` の各段落に対応する配列です。各 span を連結した文字列は、対応する `body` の本文と完全に一致させます。ビルド時にこの整合性を検証します。

```json
{
  "type": "text",
  "body": ["目的地を入力すると、乗車候補となるバス停を確認できます。"],
  "bodySpans": [["目的地を入力すると、", "乗車候補となるバス停を", "確認できます。"]]
}
```
